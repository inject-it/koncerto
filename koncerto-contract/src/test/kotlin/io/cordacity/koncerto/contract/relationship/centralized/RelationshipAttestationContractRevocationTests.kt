...
import io.cordacity.koncerto.contract.CENTRALIZED_RELATIONSHIP
...
@DisplayName("Centralized relationship attestation contract revocation tests")
class RelationshipAttestationContractRevocationTests : ContractTest() {
    ...
    fun `On relationship attestation amendment, the transaction must include the Revoke command`() {
        services.ledger {
            transaction {
                val (_, attestation) = initialize(this@ledger, CENTRALIZED_RELATIONSHIP)
